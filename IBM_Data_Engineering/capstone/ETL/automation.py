...
# Connect to MySQL
connection = mysql.connector.connect(user='root', password='MTI3Mi1rYXlzdW4t',host='127.0.0.1',database='sales')
...
dsn_uid = "myw13933"        # e.g. "abc12345"
dsn_pwd = "TZEEzKuPLs8lkUkP"      # e.g. "7dBZ3wWt9XN6$o0J"
...
def get_last_rowid():
    ...
    result_query = ibm_db.fetch_tuple(exec_query)

    return int(result_query[0])
...
def insert_records(records):
    # timestamp data is missing, column can be null
    query = "INSERT INTO sales_data(rowid,product_id,customer_id,quantity) VALUES(?,?,?,?);"
    exec_query = ibm_db.prepare(conn, query)

    for record in records:
        ibm_db.execute(exec_query, record)
